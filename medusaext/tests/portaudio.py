import platform
import ctypes
from ctypes import c_int, c_long, c_ulong, c_double, c_char_p, c_void_p, byref
...
if platform.system() == "Windows":
    ...
    libname = "libportaudio.so" # WARNING: untested
...
if __name__ == "__main__":
    cmedusa = ctypes.CDLL("libmedusa.so")
    callback = cmedusa.callback_play_tone


    pa.Pa_Initialize()
    pa.Pa_Sleep(c_int(500))

    di = pa.Pa_GetDefaultOutputDevice()
    initcount = c_int.in_dll(pa, "initializationCount_")
    print initcount

    # Pa_OpenDefaultStream (PaStream **stream, int inputChannelCount, int outputChannelCount, PaSampleFormat sampleFormat, double sampleRate, unsigned long framesPerBuffer, PaStreamCallback *streamCallback, void *userData)
    stream = c_void_p();
    pa.Pa_OpenDefaultStream(byref(stream), 0, 1, paFloat32, c_double(44100), paFramesPerBufferUnspecified, callback, None)
    ...
    # print PaHostApiInfo.__doc__
    # print PaHostErrorInfo.__doc__
    # print PaDeviceInfo.__doc__
    # print PaStreamParameters.__doc__
    # print PaStreamCallbackTimeInfo.__doc__
    # print PaStreamInfo.__doc__
